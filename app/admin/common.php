...
if (!function_exists('base64_image_content')) {
    ...
    {
        //匹配出图片的格式
        if (preg_match('/^(data:\s*[image\/]{0,6}(\w+)?;base64,)/', $base64_image_content, $result)) {
            $type = $result[2] ?? 'jpg';
            $new_file = $path . "/";
            $base64_body = substr(strstr($base64_image_content,','), 1);
            $content = base64_decode($base64_body);
            return saveImagePubZip($new_file,$content, $type, $filename);
        } else {
            return false;
        }
    }
...
if (!function_exists('saveImagePubZip')) {
    function saveImagePubZip($path, $content, $type = 'jpg', $filename = '')
    {
        ...
        if (empty($filename)) {
            $filename = intval(microtime(true) * 10000) . random();
        }else{
            $filename .= '-'.intval(microtime(true) * 10000) . mt_rand(1000,9999);
        }
        $path = $path . $filename . ".{$type}";
        if (file_put_contents(public_path().$path, $content)) {
            return $path;
        } else {
            return false;
        }
