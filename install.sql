...
-- extra mysql --
ALTER TABLE `xwx_book` ADD COLUMN `tags_id`  int(11) UNSIGNED NULL DEFAULT 0 COMMENT '分类ID' AFTER `is_copyright`;

ALTER TABLE `xwx_book` MODIFY COLUMN `tags`  varchar(100) CHARACTER SET utf8mb4 COLLATE utf8mb4_general_ci NULL DEFAULT '' COMMENT '标签' AFTER `delete_time`;

ALTER TABLE `xwx_user_order`
MODIFY COLUMN `pay_type`  tinyint(4) NULL DEFAULT 1 COMMENT '0为未知，1是支付宝，2是微信，3是QQ，4为卡密，5为PayPal' AFTER `status`,
ADD COLUMN `order_type`  tinyint(2) UNSIGNED NULL DEFAULT 0 COMMENT '订单类型，默认0无类型 1 vip充值订单 2 金币充值订单' AFTER `expire_time`,
ADD COLUMN `wallet`  decimal(10,0) UNSIGNED NULL DEFAULT 0 COMMENT 'order_type=1时为VIP天数，2时充值金币数' AFTER `order_type`;
