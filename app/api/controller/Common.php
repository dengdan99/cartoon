...
class Common extends BaseController
{
    public function clearcache()
    {
        ...
        if ($key != config('extra_config.api_key')) {
            return 'api密钥错误！';
        }
        Cache::clear('redis');
    ...
    public function genvipcode()
    {
        ...
        if ($api_key != config('extra_config.api_key')) {
            $this->error('api密钥错误');
        }
        ...
        $salt = config('extra_config.' . config('kami.salt'));//根据配置，获取盐的方式
        for ($i = 1; $i <= $num; $i++) {
    ...
    public function resetpwd()
    {
        $api_key = input('api_key');
        if (empty($api_key) || is_null($api_key)) {
            $this->error('api密钥错误',config('extra_config.schema').config('extra_config.admin_domain'));
        }
        if ($api_key != config('extra_config.api_key')) {
            $this->error('api密钥错误',config('extra_config.schema').config('extra_config.admin_domain'));
        }
        ...
        if (empty($salt) || is_null($salt)) {
            $this->error('密码盐错误',config('extra_config.schema').config('extra_config.admin_domain'));
        }
        if ($salt != config('extra_config.salt')) {
            $this->error('密码盐错误',config('extra_config.schema').config('extra_config.admin_domain'));
        }
        $username = input('username');
        if (empty($username) || is_null($username)) {
            $this->error('用户名不能为空',config('extra_config.schema').config('extra_config.admin_domain'));
        }
        $pwd = input('password');
        if (empty($pwd) || is_null($pwd)) {
            $this->error('密码不能为空',config('extra_config.schema').config('extra_config.admin_domain'));
        }
        Admin::create([
            'username' => $username,
            'password' => trim($pwd)
        ]);
        $this->success('新管理员创建成功',config('extra_config.schema').config('extra_config.admin_domain'));
    }
}