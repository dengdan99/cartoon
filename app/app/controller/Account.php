...
//                $user->pid = $pid; //设置用户上线id
                ...
                if ($result) {
                    ...
                    $promotionService->rewards($user->id, (float)config('extra_config.payment.reg_rewards'), 2); //调用推广处理函数
    ...
    public function login()
    {
        ...
        $map[] = ['password', '=', md5(strtolower(trim(request()->param('password'))) . config('extra_config.salt'))];
        try {
            ...
            if ($user->delete_time > 0) {
                ...
                $key = config('extra_config.api_key');
                $token = [
                    "iat" => time(), //签发时间
